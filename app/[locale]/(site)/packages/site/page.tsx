...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "metadata.packageSite" });
...
const sectionIcons = [Code2, Smartphone, Mail, Globe, Megaphone, BarChart3];
const linkHrefs = ["/web/landing-pages", "/web/business-websites"];

export default async function SitePackagePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);

  const t = await getTranslations({ locale, namespace: "packageSitePage" });
  const tb = await getTranslations({ locale, namespace: "breadcrumbs" });

  const breadcrumbItems = [
    { label: tb("home"), href: "/" },
    { label: tb("packages"), href: "/packages" },
    { label: tb("start"), href: "/packages/site" },
  ];
          ...
          <section className="py-16 md:py-20">
            <div className="container mx-auto max-w-5xl px-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {sectionIcons.map((Icon, sectionIndex) => (
                  <div
                    key={sectionIndex}
                    ...
                    <ul className="space-y-4">
                      {Array.from({ length: 5 }, (_, itemIndex) => (
                        ...
                        >
                          ...
                          <span>{t(`sections.${sectionIndex}.items.${itemIndex}`)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {Array.from({ length: 4 }, (_, i) => (
                  ...
                  >
                    ...
                    <p className="text-[#1A1A1A]/60">{t(`idealFor.${i}.desc`)}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="container mx-auto max-w-4xl">
              <div className="bg-[#FFD166] border-2 border-[#1A1A1A] rounded-3xl p-8 md:p-12 text-center shadow-[8px_8px_0px_0px_#1A1A1A]">
                ...
                <Button
                  ...
                  asChild
                >
                  <Link href="/packages/leads">{t("upgradeButton")}</Link>
                </Button>
              </div>
            </div>
          </section>
